...
public:
	NmeaParsingNode() : privnode("~"), serialPort(""), baudRate(0), tcpHostname(""), tcpPort(0), mode("")
	{
		privnode.getParam("mode",mode);
		privnode.getParam("serialPort",serialPort);
		privnode.getParam("hostname",tcpHostname);
		privnode.getParam("baudRate",baudRate);
		privnode.getParam("port",tcpPort);

		ROS_INFO("Mode: %s | Serial port: %s | Hostname: %s | Baud rate: %d | TCP port: %d", mode.c_str(), serialPort.c_str(), tcpHostname.c_str(), baudRate, tcpPort);

		kill = false;

		if(mode == "tcp" && tcpHostname.size() > 0 && tcpPort > 0) {
			tcpMode = true;
		} else if(mode == "serial" && serialPort.size() > 0 && baudRate > 0) {
			tcpMode = false;
		} else {
			// bad stuff
			// TODO exception
...
private:
	...
	bool openSerial() {
		iface.reset(new NPNSerial(serialPort,baudRate));
		return true;
	}

	bool openTcp() {
		iface.reset(new NPNTcp(tcpHostname,tcpPort));
		return true;
	}
